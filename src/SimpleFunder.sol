...
contract SimpleFunder is EIP712, Ownable, IFunder {
    ...
    bytes32 constant WITHDRAWAL_TYPE_HASH =
        keccak256("Withdrawal(address token,address recipient,uint256 amount,uint256 deadline,uint256 nonce)");
    ...
    function withdrawTokensWithSignature(
        ...
        bytes32 digest =
            _hashTypedData(keccak256(abi.encode(WITHDRAWAL_TYPE_HASH, token, recipient, amount, deadline, nonce)));
    ...
    function fund(address, bytes32 digest, ICommon.Transfer[] memory transfers, bytes memory funderSignature)
        external
        override
    {
    ...
    function fund(bytes32 digest, ICommon.Transfer[] memory transfers, bytes memory funderSignature) public override {
