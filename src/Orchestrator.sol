...
contract Orchestrator is IOrchestrator, EIP712, CallContextChecker, ReentrancyGuardTransient {
    ...
    function simulateExecute(bool isStateOverride, uint256 combinedGasOverride, bytes calldata encodedIntent)
        external
        payable
        returns (uint256)
    {
        // If Simulation Fails, then it will revert here.
        (uint256 gUsed, bytes4 err) = _execute(encodedIntent, combinedGasOverride, _SIMULATION_MODE_FLAG);
    ...
    function _extractIntent(bytes calldata encodedIntent) internal view virtual returns (Intent calldata i) {
    ...
    function _extractPreCall(bytes calldata encodedPreCall) internal virtual returns (SignedCall calldata p) {
        assembly ("memory-safe") {
        ...
        returns (uint256 gUsed, bytes4 err)
    {
        ...
        assembly ("memory-safe") {
            ...
            if iszero(err) {
                ...
                selfCallSuccess := call(g, address(), 0, add(m, 0x1c), add(encodedIntent.length, 0x24), 0x00, 0x20)
    ...
    function selfCallPayVerifyCall537021665() public payable {
        ...
        if (i.encodedPreCalls.length != 0) {
            _handlePreCalls(eoa, i.payer, flags, i.encodedPreCalls);
        }
        ...
        assembly ("memory-safe") {
            ...
            if iszero(call(gas(), eoa, 0, add(0x20, data), mload(data), 0x00, 0x20)) {
                ...
                if iszero(mload(0x00)) {
                    mstore(0x00, shl(224, 0x6c9d47e8))
                } // `CallError()`.
                return(0x00, 0x20)
            }
        }
    ...
    function _handlePreCalls(address parentEOA, address payer, uint256 flags, bytes[] calldata encodedPreCalls)
        internal
        virtual
    {
    ...
    function _pay(bytes32 keyHash, bytes32 digest, Intent calldata i) internal virtual {
        ...
        uint256 requiredBalanceAfter =
            Math.saturatingAdd(TokenTransferLib.balanceOf(i.paymentToken, i.paymentRecipient), paymentAmount);
    ...
    function _executionDataHash(bytes calldata executionData) internal view virtual returns (bytes32) {
