...
contract Simulator {
    ...
    function _callOrchestrator(address oc, bool isStateOverride, bytes memory data) internal returns (uint256 gasUsed) {
        assembly ("memory-safe") {
    ...
    function _callOrchestratorCalldata(
        ...
        bytes memory data = abi.encodeWithSignature(
            "simulateExecute(bool,uint256,bytes)", isStateOverride, combinedGasOverride, encodedIntent
        );
    ...
    function _callOrchestratorMemory(
        ...
        bytes memory data = abi.encodeWithSignature(
            "simulateExecute(bool,uint256,bytes)", isStateOverride, combinedGasOverride, abi.encode(u)
        );
        return _callOrchestrator(oc, isStateOverride, data);
    }
    ...
    function _callMulticall3(
        ...
        uint256 combinedGasOverride,
        ICommon.Intent memory u
    ) internal freeTempMemory returns (uint256 gasUsed, uint256 multicall3Gas, bytes memory lastReturnData) {
        ...
        allCalls[calls.length] = IMulticall3.Call3({target: oc, allowFailure: true, callData: orchestratorData});
    ...
    function simulateMulticall3CombinedGas(
        ...
        (gasUsed, multicall3Gas, errorData) = _callMulticall3(multicall3, calls, oc, false, type(uint256).max, u);
        ...
        while (true) {
            (gasUsed, multicall3Gas, errorData) = _callMulticall3(multicall3, calls, oc, false, 0, u);
    ...
    function simulateMulticall3V1Logs(
        ...
        (gasUsed, multicall3Gas, combinedGas) = simulateMulticall3CombinedGas(
            multicall3, calls, oc, paymentPerGasPrecision, paymentPerGas, combinedGasIncrement, encodedIntent
        );
