...
library LibERC7821 {
    ...
    function execute(bytes32 _mode, bytes calldata _executionData) internal {
        uint256 id = executionModeId(_mode);
        if (id == 3) return executeBatchOfBatches(_mode, _executionData);
        Call[] calldata calls;
        ...
        assembly ("memory-safe") {
            ...
            opData.length := 0
            let o := add(_executionData.offset, calldataload(_executionData.offset))
            ...
            if gt(eq(id, 2), gt(0x40, calldataload(_executionData.offset))) {
                let q := add(_executionData.offset, calldataload(add(0x20, _executionData.offset)))
        ...
        execute(_mode, _executionData, calls, opData);
    }

    /// @dev Provided for execution mode support detection.
    function supportsExecutionMode(bytes32 _mode) internal pure returns (bool) {
        return executionModeId(_mode) != 0;
    }
    ...
    function executionModeId(bytes32 _mode) internal pure returns (uint256 id_) {
        ...
        assembly ("memory-safe") {
            let m := and(shr(mul(22, 8), _mode), 0xffff00000000ffffffff)
            id_ := eq(m, 0x01000000000000000000) // 1.
            id_ := or(shl(1, eq(m, 0x01000000000078210001)), id_) // 2.
            id_ := or(mul(3, eq(m, 0x01000000000078210002)), id_) // 3.
        }
    }

    /// @dev For execution of a batch of batches.
    function executeBatchOfBatches(bytes32 _mode, bytes calldata _executionData) internal {
        // Replace with `0x0100________78210001...` while preserving optional and reserved fields.
        _mode ^= bytes32(uint256(3 << (22 * 8))); // `2 XOR 3 = 1`.
        (uint256 n, uint256 o, uint256 e) = (0, 0, 0);
        assembly ("memory-safe") {
            let j := calldataload(_executionData.offset)
            let t := add(_executionData.offset, j)
            n := calldataload(t) // `batches.length`.
            o := add(0x20, t) // Offset of `batches[0]`.
            e := add(_executionData.offset, _executionData.length) // End of `executionData`.
            ...
            if or(shr(64, j), or(lt(_executionData.length, 0x20), gt(add(o, shl(5, n)), e))) {
    ...
    function execute(bytes32 _mode, bytes calldata _executionData, Call[] calldata _calls, bytes calldata _opData)
        internal
    {
        // Silence compiler warning on unused variables.
        _mode = _mode;
        _executionData = _executionData;
    ...
    function execute(Call[] calldata calls, bytes32 extraData) internal {
        unchecked {
            uint256 i;
            if (calls.length == uint256(0)) return;
            do {
                (address to, uint256 value, bytes calldata data) = get(calls, i);
                execute(to, value, data, extraData);
    ...
    /// Reverts and bubbles up error if any call fails.
    /// `extraData` can be any supplementary data (e.g. a memory pointer, some hash).
    function execute(address to, uint256 value, bytes calldata data, bytes32 extraData) internal {
        assembly ("memory-safe") {
    ...
    function get(Call[] calldata calls, uint256 i)
        internal
        view
