...
import {Call, LibERC7821} from "../erc7821/LibERC7821.sol";
...
bytes32 constant ANY_KEYHASH = 0x3232323232323232323232323232323232323232323232323232323232323232;
...
library GuardedExecutorLib {
    ...
    function execute(Call[] calldata calls, bytes32 keyHash) internal {
        ...
        incrementSpent(spends.spends[address(0)], address(0), totalNativeSpend);
    ...
    function execute(address target, uint256 value, bytes calldata data, bytes32 keyHash) internal {
        ...
        LibERC7821.execute(target, value, data, keyHash);
    }
        ...
        checkKeyHashIsNonZero(keyHash)
    {
        ...
        if (tokenSpends.periods.remove(uint8(period))) {
            if (tokenSpends.periods.length() == uint256(0)) {
    ...
    function canExecute(bytes32 keyHash, address target, bytes calldata data) internal view returns (bool) {
        ...
        if (c.length() != 0) {
            ...
            if (c.contains(packCanExecute(ANY_TARGET, fnSel))) return true;
            if (c.contains(packCanExecute(ANY_TARGET, ANY_FN_SEL))) {
                return true;
            }
        }
        c = getGuardedExecutorKeyStorage(ANY_KEYHASH).canExecute;
        if (c.length() != 0) {
            if (c.contains(packCanExecute(target, fnSel))) return true;
            if (c.contains(packCanExecute(target, ANY_FN_SEL))) return true;
            if (c.contains(packCanExecute(ANY_TARGET, fnSel))) return true;
        ...
        if (checkCall(keyHash, keyHash, target, target, data)) return true;
        if (checkCall(keyHash, keyHash, ANY_TARGET, target, data)) return true;
        if (checkCall(ANY_KEYHASH, keyHash, target, target, data)) return true;
        if (checkCall(ANY_KEYHASH, keyHash, ANY_TARGET, target, data)) {
            return true;
        }
        return false;
    }
