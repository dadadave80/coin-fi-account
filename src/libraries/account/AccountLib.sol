...
import {LibNonce} from "@coinfi-account-lib/LibNonce.sol";
import {LibTStack} from "@coinfi-account-lib/LibTStack.sol";
import {TokenTransferLib} from "@coinfi-account-lib/TokenTransferLib.sol";
import {EIP712Lib} from "@coinfi-account-lib/eip712/EIP712Lib.sol";
import {LibERC7821} from "@coinfi-account-lib/erc7821/LibERC7821.sol";
import {Call, GuardedExecutor} from "@coinfi-account-lib/guardedExecutor/GuardedExecutor.sol";
import {GuardedExecutorLib} from "@coinfi-account-lib/guardedExecutor/GuardedExecutorLib.sol";
import {SpendStorage, Unauthorized} from "@coinfi-account-lib/guardedExecutor/GuardedExecutorLib.sol";
import {ICommon} from "@coinfi-account/interfaces/ICommon.sol";
...
bytes32 constant DOMAIN_TYPEHASH = EIP712Lib._DOMAIN_TYPEHASH;

/// @dev For ERC1271 replay-safe hashing.
bytes32 constant SIGN_TYPEHASH = keccak256("ERC1271Sign(bytes32 digest)");
...
uint16 constant MULTICHAIN_NONCE_PREFIX = 0xc1d0;
...
bytes32 constant _UPGRADE_HOOK_ID = keccak256("ITHACA_ACCOUNT_UPGRADE_HOOK_ID");

/// @dev This transient slot must be set to `_UPGRADE_HOOK_ID` before `upgradeHook` can be processed.
bytes32 constant _UPGRADE_HOOK_GUARD_TRANSIENT_SLOT =
...
uint256 constant _KEYHASH_STACK_TRANSIENT_SLOT = uint256(keccak256("_KEYHASH_STACK_TRANSIENT_SLOT")) - 1;

/// @dev General capacity for enumerable sets,
/// to prevent off-chain full enumeration from running out-of-gas.
uint256 constant _CAP = 512;
...
library AccountLib {
    ...
    function getKeyExtraStorage(bytes32 keyHash) internal view returns (KeyExtraStorage storage $) {
        bytes32 s = getAccountStorage().keyExtraStorage[keyHash].slot();
        assembly ("memory-safe") {
            $.slot := s
        }
    }
    ...
    function hashTypedDataOnlyVerifyingContract(bytes32 structHash) internal view returns (bytes32 digest) {
        assembly ("memory-safe") {
    ...
    function isValidSignature(bytes32 digest, bytes calldata signature) internal view returns (bytes4) {
        ...
        digest = hashTypedDataOnlyVerifyingContract(replaySafeDigest);

        (bool isValid, bytes32 keyHash) = unwrapAndValidateSignature(digest, signature);
        if (LibBit.and(keyHash != 0, isValid)) {
            isValid = isSuperAdmin(keyHash) || getKeyExtraStorage(keyHash).checkers.contains(msg.sender);
        }
    ...
    /// @dev Sets the label.
    function setLabel(string calldata newLabel) internal onlyThis {
        getAccountStorage().label.set(bytes(newLabel));
        emit LabelSet(newLabel);
    }

    /// @dev Revokes the key corresponding to `keyHash`.
    function revoke(bytes32 keyHash) internal onlyThis {
        removeKey(keyHash);
        emit Revoked(keyHash);
    }

    /// @dev Authorizes the key.
    function authorize(Key memory key) internal returns (bytes32 keyHash) {
        keyHash = addKey(key);
    ...
    function setSignatureCheckerApproval(bytes32 keyHash, address checker, bool isApproved) internal onlyThis {
        if (getAccountStorage().keyStorage[keyHash].isEmpty()) {
            revert KeyDoesNotExist();
        }
        getKeyExtraStorage(keyHash).checkers.update(checker, isApproved, _CAP);
    ...
    function invalidateNonce(uint256 nonce) internal onlyThis {
        LibNonce.invalidate(getAccountStorage().nonceSeqs, nonce);
    ...
    function getNonce(uint192 seqKey) internal view returns (uint256) {
        return LibNonce.get(getAccountStorage().nonceSeqs, seqKey);
    }

    /// @dev Returns the label.
    function label() internal view returns (string memory) {
        return string(getAccountStorage().label.get());
    }

    /// @dev Returns the number of authorized keys.
    function keyCount() internal view returns (uint256) {
        return getAccountStorage().keyHashes.length();
    }

    /// @dev Returns the authorized key at index `i`.
    function keyAt(uint256 i) internal view returns (Key memory) {
        return getKey(getAccountStorage().keyHashes.at(i));
    }

    /// @dev Returns the key corresponding to the `keyHash`. Reverts if the key does not exist.
    function getKey(bytes32 keyHash) internal view returns (Key memory key) {
        bytes memory data = getAccountStorage().keyStorage[keyHash].get();
    ...
    function getKeys() internal view returns (Key[] memory keys, bytes32[] memory keyHashes) {
        ...
        for (uint256 i = 0; i < totalCount; i++) {
            bytes32 keyHash = getAccountStorage().keyHashes.at(i);
    ...
    function getContextKeyHash() internal view returns (bytes32) {
    ...
    function approvedSignatureCheckers(bytes32 keyHash) internal view returns (address[] memory) {
        return getKeyExtraStorage(keyHash).checkers.values();
    }
    ...
    function computeDigest(Call[] calldata calls, uint256 nonce) internal view returns (bytes32 result) {
        ...
        bytes32 structHash =
            EfficientHashLib.hash(uint256(EXECUTE_TYPEHASH), LibBit.toUint(isMultichain), uint256(a.hash()), nonce);
        return isMultichain ? EIP712Lib.hashTypedDataSansChainId(structHash) : EIP712Lib.hashTypedData(structHash);
    }
    ...
    function addKey(Key memory key) internal returns (bytes32 keyHash) {
        // `keccak256(abi.encode(key.keyType, keccak256(key.publicKey)))`.
        keyHash = hash(key);
        AccountStorage storage $ = getAccountStorage();
        $.keyStorage[keyHash].set(abi.encodePacked(key.publicKey, key.expiry, key.keyType, key.isSuperAdmin));
        $.keyHashes.add(keyHash);
    }

    /// @dev Removes the key corresponding to the `keyHash`. Reverts if the key does not exist.
    function removeKey(bytes32 keyHash) internal {
        AccountStorage storage $ = getAccountStorage();
    ...
    function checkAndIncrementNonce(uint256 nonce) internal {
        if (msg.sender != getAccountStorage().ORCHESTRATOR) {
            revert Unauthorized();
        }
        LibNonce.checkAndIncrement(getAccountStorage().nonceSeqs, nonce);
    }

    /// @dev Pays `paymentAmount` of `paymentToken` to the `paymentRecipient`.
    function pay(uint256 paymentAmount, bytes32 keyHash, bytes32 intentDigest, bytes calldata encodedIntent) internal {
        ICommon.Intent calldata intent;
        ...
        if (!LibBit.and(
                msg.sender == getAccountStorage().ORCHESTRATOR,
        ...
        if (intent.payer == address(this)) {
            if (getAccountStorage().paymasterNonces[intentDigest]) {
                revert PaymasterNonceError();
            }
            getAccountStorage().paymasterNonces[intentDigest] = true;

            (bool isValid, bytes32 k) = unwrapAndValidateSignature(intentDigest, intent.paymentSignature);
        ...
        // Increase spend.
        if (!(keyHash == bytes32(0) || isSuperAdmin(keyHash))) {
            SpendStorage storage spends = GuardedExecutorLib.getGuardedExecutorKeyStorage(keyHash).spends;
            GuardedExecutorLib.incrementSpent(spends.spends[intent.paymentToken], intent.paymentToken, paymentAmount);
        }
    ...
    /// @dev For ERC7821.
    function execute(bytes32, bytes calldata, Call[] calldata calls, bytes calldata opData) internal {
        ...
        if (msg.sender == getAccountStorage().ORCHESTRATOR) {
            ...
            LibERC7821.execute(calls, _keyHash);
        ...
        if (opData.length == uint256(0)) {
            ...
            return LibERC7821.execute(calls, bytes32(0));
        }
        ...
        (bool isValid, bytes32 keyHash) =
            unwrapAndValidateSignature(computeDigest(calls, nonce), LibBytes.sliceCalldata(opData, 0x20));
        ...
        LibERC7821.execute(calls, keyHash);
        LibTStack.TStack(_KEYHASH_STACK_TRANSIENT_SLOT).pop();
    }
    ...
    function isSuperAdmin(bytes32 keyHash) internal view returns (bool) {
        LibBytes.BytesStorage storage s = getAccountStorage().keyStorage[keyHash];
    ...
    function getGuardedExecutorKeyStorageSeed(bytes32 keyHash) internal view returns (bytes32) {
        return getAccountStorage().keyExtraStorage[keyHash].slot();
    }
    ...
    /// @dev For EIP712.
    function domainNameAndVersion() internal view returns (string memory name, string memory version) {
        name = "IthacaAccount";
        version = "0.5.10";
    }
